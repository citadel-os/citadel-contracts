...
contract CitadelStorageV2 is Ownable {
    ...
    // public functions
    function liteGrid(uint256[] calldata _pilotIds, uint256 _gridId, uint8 _factionId) public onlyOwner {
        //todo ensure zero out is correct if adding to
        //
        uint256 citadelId = grid[_gridId];
        citadel[citadelId].gridId = _gridId;
        citadel[citadelId].factionId = _factionId;
        citadel[citadelId].timeOfLastClaim = 0;
        ...
        ) = fleetEngine.getTrainedFleet(citadelId);
        fleetDebit[citadelId].sifGattaca = int256(trainedSifGattaca);
        fleetDebit[citadelId].mhrudvogThrot = int256(trainedMhrudvogThrot);
        fleetDebit[citadelId].drebentraakht = int256(trainedDrebentraakht);
    ...
    function dimGrid(uint256 _citadelId, uint256 _pilotId) public onlyOwner {
    ...
    function claim(uint256 _citadelId) public onlyOwner returns (uint256) {
